    ...
    case $ARCH in
        x86_64|i386|i486|i586|i686)
            if [[ $V7 == 1 && $BOOT_MODE == "BIOS" ]]; then
                IMG_URL="https://github.com/ipsat98/MikV7-Archive/releases/download/$VERSION/chr-$VERSION-legacy-bios.img-dev.zip"
            else
                IMG_URL="https://github.com/ipsat98/MikV7-Archive/releases/download/$VERSION/chr-$VERSION.img-dev.zip"
            fi
            ;; 
        aarch64)
