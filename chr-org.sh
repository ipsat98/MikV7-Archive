...
select_version() {
    ...
    while true; do
        ...
        case $version_choice in
            1) 
                VERSION=$(http_get "https://upgrade.mikrotik.com/routeros/NEWESTa7.stable" | cut -d' ' -f1)
                V7=1
                ;;
            2) 
                VERSION=$(http_get "https://upgrade.mikrotik.com/routeros/NEWESTa7.testing" | cut -d' ' -f1)
                V7=1
                ;;
            3)
                ...
                VERSION=$(http_get "https://upgrade.mikrotik.com/routeros/NEWEST6.long-term" | cut -d' ' -f1)
                V7=0
                ;;
            4)
                if [[ "$ARCH" == "aarch64" ]]; then
                ...
                VERSION=$(http_get "https://upgrade.mikrotik.com/routeros/NEWEST6.stable" | cut -d' ' -f1)
                V7=0
                ;;
            *)
...
download_image(){
    case $ARCH in
        x86_64|i386|i486|i586|i686)
            ...
            ;; 
        aarch64)
             IMG_URL="https://download.mikrotik.com/routeros/$VERSION/chr-$VERSION-arm64.img.zip"
            ;; 
        *)
