...
select_version() {
    ...
    while true; do
        ...
        case $version_choice in
            1) 
                VERSION=$(http_get "https://upgrade.routeros.ltd/routeros/NEWESTa7.stable" | cut -d' ' -f1)
                V7=1
                ;;
            2) 
                VERSION=$(http_get "https://upgrade.routeros.ltd/routeros/NEWESTa7.testing" | cut -d' ' -f1)
                V7=1
                ;;
            3)
                ...
                VERSION=$(http_get "https://upgrade.routeros.ltd/routeros/NEWEST6.long-term" | cut -d' ' -f1)
                V7=0
                ;;
            4)
                if [[ "$ARCH" == "aarch64" ]]; then
                ...
                VERSION=$(http_get "https://upgrade.routeros.ltd/routeros/NEWEST6.stable" | cut -d' ' -f1)
                V7=0
                ;;
            *)
    ...
    case $ARCH in
        x86_64|i386|i486|i586|i686)
            if [[ $V7 == 1 && $BOOT_MODE == "BIOS" ]]; then
                IMG_URL="https://upgrade.routeros.ltd/routeros/$VERSION/chr-$VERSION-bios.img.zip"
            else
                IMG_URL="https://upgrade.routeros.ltd/routeros/$VERSION/chr-$VERSION.img.zip"
            fi
            ;; 
        aarch64)
             IMG_URL="https://upgrade.routeros.ltd/routeros/$VERSION/chr-$VERSION-arm64.img.zip"
            ;; 
        *)
